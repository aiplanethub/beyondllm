...
def default_llm():
    ...
    if not api_key:
        raise EnvironmentError(
            "GOOGLE_API_KEY must be set in the environment variables."
        )
...
class Generate:
    ...
    def pipeline(self):
        self.CONTEXT = [
            ...
            for node_with_score in self.retriever.retrieve(self.question)
        ]
    ...
    def get_context_relevancy(self, llm=None):
        ...
        for context in self.CONTEXT:
            score_str = llm.predict(
                CONTEXT_RELEVENCE.format(question=self.question, context=context)
            )
    ...
    def get_answer_relevancy(self, llm=None):
        ...
        try:
            score_str = llm.predict(
                ANSWER_RELEVENCE.format(question=self.question, context=self.RESPONSE)
            )
            score = float(extract_number(score_str))
    ...
    def get_groundedness(self, llm=None):
        ...
        try:
            ...
            for statement in statements:
                ...
                score_response = llm.predict(
                    GROUNDEDNESS.format(statement=statement, context=self.CONTEXT)
                )
    ...
    def get_ground_truth(self, answer: str, llm=None):
        ...
        if answer is None:
            print(
                "Error: Missing 'answer' parameter. Usage: get_ground_truth(answer=\"your_answer_here\")."
            )
            return
        score_str = llm.predict(
            GROUND_TRUTH.format(ground_truth=answer, generated_response=self.RESPONSE)
        )
    ...
    def get_faithfulness(self, llm=None):
        ...
        try:
            score_str = llm.predict(
                FAITHFULNESS.format(statement=self.RESPONSE, context=self.CONTEXT)
            )
            score = float(extract_number(score_str))
    ...
    def get_correctness(self, llm=None):
        ...
        try:
            score_str = llm.predict(
                CORRECTNESS.format(statement=self.RESPONSE, context=self.CONTEXT)
            )
            score = float(extract_number(score_str))
